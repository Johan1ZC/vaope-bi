...
# =========== CONFIGURA AQUÍ ===========
URL = "https://odoo-stage.colaborativa.pe/api/vendor_bills"    # <-- PON TU URL
...
MAX_PAGES  = 1000             # tope de seguridad
LIMIT      = 1000              # si el backend capa a 200, usa 200
...
def fetch_all_pages():
    ...
    while True:
        ...
        for r in chunk:
            kid = r.get(ID_FIELD)
            if kid is None:
                # si no hay ID_FIELD, agregamos y confiamos en len()
                nuevos.append(r)
            elif kid not in seen_keys:
                seen_keys.add(kid)
                nuevos.append(r)
