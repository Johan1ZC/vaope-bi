...
-- =========================
-- 5) SNAPSHOTS (último día de mes de los últimos 6 meses)
-- =========================

/* FactSnapShot: solo últimos 12 fin de mes, sin CTE */
INSERT INTO FactSnapShot
  (FechaID, EmpleadoID, EstructuraID, EsActivo, Antiguedad, Fuente)
SELECT
  f.FechaID,
  e.EmpleadoID,
  /* Asigna estructura de forma estable y repartida */
  ((e.EmpleadoID - 1) % (SELECT COUNT(*) FROM DimEstructura)) + 1  AS EstructuraID,
  /* Activo si el snapshot cae entre ingreso y cese (o sin cese) */
  CASE
    WHEN e.FecIngreso <= f.Fecha
     AND (e.FecCese IS NULL OR e.FecCese >= f.Fecha) THEN 1
    ELSE 0
  END AS EsActivo,
  /* Antigüedad en meses al snapshot, mínimo 0 */
  CASE
    WHEN e.FecIngreso > f.Fecha THEN 0
    ELSE TIMESTAMPDIFF(MONTH, e.FecIngreso, f.Fecha)
  END AS Antiguedad,
  'SEED' AS Fuente
FROM DimFecha f
JOIN DimEmpleado e
WHERE f.Fecha = LAST_DAY(f.Fecha)  -- solo fin de mes
  AND f.Fecha BETWEEN
      DATE_SUB(LAST_DAY(CURDATE()), INTERVAL 11 MONTH)  -- desde hace 11 meses
      AND LAST_DAY(CURDATE())                           -- hasta fin de mes actual
...
UNION ALL
SELECT 'FactSnapShot', COUNT(*) FROM FactSnapShot;
