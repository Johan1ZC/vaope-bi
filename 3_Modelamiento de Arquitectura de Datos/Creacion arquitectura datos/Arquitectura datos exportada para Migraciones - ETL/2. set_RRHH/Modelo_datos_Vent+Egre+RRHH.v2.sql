/* =======================================================================
   Huella: JZ-VAOPE-DW-SET_RRHH-002
   Proyecto: VAOPE – Data Warehouse (RRHH)
   ...
   Dependencias: DimFecha creada peviamente - Privilegios CREATE/ALTER/INDEX.
   Descripción:
     - Crea dimensiones y hechos clave (DimEmpleado, DimEstructura, DimEstadosRRHH,
       FactAsistencia, FactEventosRRHH)
   ...
   Métricas rápidas del DDL:
     ...
     - Foreign keys: 9
   ADR relacionado:
     - ADR-003: Relaciones de fecha activa (EstructuraDesdeID) e inactiva (EstructuraHastaID) para eventosRRHH.
     - ADR-005: Índices compuestos en DimEstructura, DimEstadosRRHH, FactAsistencia y FactEventosRRHH.
   Convención de nombres:
     - Dim* (dimensiones), Fact* (hechos), *_Detalle (puentes), fk_* (FK), uq_* (UK), ix_* (index).
   Propósito:
     - Estandarizar el modelo estrella para RRHH vinculados a Estructura, Empleado,
       Estados con claves de negocio y surrogate keys.
   ======================================================================= */
