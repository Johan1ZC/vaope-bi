...
a.payment_method_id
from vaope_qa.sales a
-- inner join vaope.lead_paginaweb_muestra d on a.id = d.id  -- DESACTIVAR EN PRODUCCION
left join vaope_qa.products c on a.product_id = c.id
left join (select a.* from vaope_qa.product_liquidation_detail a
inner join (select sale_id,max(id) max_id,count(1) Q from vaope_qa.product_liquidation_detail group by sale_id) b on a.sale_id = b.sale_id and a.id = b.max_id) e on a.id = e.sale_id and a.payment_method_id = e.payment_method_id
group by 
...
LEFT JOIN vaope_qa.payment_methods b
...
WITH b AS (
  ...
  FROM vaope_qa.sale_payments sp
